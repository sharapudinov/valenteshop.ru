...
for ($i = 0; $i < count($f); $i++){
	if ($f[$i][0] == "-") {
		...
		echo "<span>".$f[$i]."</span>";
		if ($f[$i][0] == "+"){
			echo"<span><input placeholder='".$f[$i]."' class='cme_txt cme_txt_err' type='text' maxlength='150'></span>";
		}else{
...
?>
	<span class="cme_btn_place">
		<button onClick="javascript: _gaq.push(['_trackPageview','/callback/']); yaCounter22604536.reachGoal('callback');" class="cme_btn"><?php echo $button; ?></button>
	</span>
