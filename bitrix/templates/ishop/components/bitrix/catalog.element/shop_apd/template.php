...
<?foreach($prop_sku  as $prop_sku_code){?>
	<?if(!empty($arResult['PROPERTIES'][$prop_sku_code]['VALUE']) && in_array($prop_sku_code, $proper)):?>
	
		<?if($arResult['PROPERTIES'][$prop_sku_code]['PROPERTY_TYPE'] == "S"):?>
			<?/*print_r($arResult['PROPERTIES'][$prop_sku_code]);*/?> 
		<tr valign="top">
		<td><?=$arResult['PROPERTIES'][$prop_sku_code]['NAME']?>:</td>
		<td>
		<div class="lineForm check_select">
		<select id="<?=$prop_sku_code;?>_<?=$arResult['PROPERTIES'][$prop_sku_code]['PROPERTY_VALUE_ID'][$propelid_key]?>" class="sel80" name="prop[<?echo $prop_sku_code?>]">
		<?$code_clrs = array();?>
			<?$iii = 0;
			foreach($arResult['PROPERTIES'][$prop_sku_code]['VALUE'] as $propelid_key => $propelid){?>
			<?
			...
			?>
				<?if($iii++ == 0){?>
					<option value="<?=$arResult['PROPERTIES'][$prop_sku_code]['DESCRIPTION'][$propelid_key]?>/<?=$propelid?>" selected><?=$propelid?></option>
				<?}else{?>
					<option value="<?=$arResult['PROPERTIES'][$prop_sku_code]['DESCRIPTION'][$propelid_key]?>/<?=$propelid?>"><?=$propelid?></option>
				<?}?>
			<?}?>
...
<div id="button_colors<?=$prop_sku_code?>_block_<?=$arResult['ID'];?>" class="button_colors_block">		
	<div class="button_colors">Выберите цвет</div>
</div>		
<div id="frame_colors<?=$prop_sku_code?>_<?=$arResult['ID'];?>" class="frame_colors">
<div class="frame_colors_mass" style="display:none;"><?=serialize($code_clrs);?></div>
<div class="frame_colors_mass_prop" style="display:none;"><?=serialize($arResult['PROPERTIES']);?></div>
<?$ic = 1;?> 
...
<?endif;?>		
		...
		<?elseif($arResult['PROPERTIES'][$prop_sku_code]['PROPERTY_TYPE'] == "E"):?>
		
		<tr valign="top">
		<td><?=$arResult['PROPERTIES'][$prop_sku_code]['NAME']?>:</td>
		<td>
		<div class="lineForm check_select">
		<select id="<?=$prop_sku_code;?>_<?=$arResult['ID']?>" class="sel80" name="prop[<?echo $prop_sku_code?>]">
										
			<?$arFilter = Array("IBLOCK_ID"=>'29', "ID" =>  $arResult['PROPERTIES'][$prop_sku_code]['VALUE']);
...
<tr valign="top">
	<td></td>
		<td>
<div class="colors_boxbox">		
	<div id="button_colors_block_<?=$arResult['ID'];?>" class="button_colors_block">		
		<div class="button_colors">Выберите цвет</div>
	</div>
	<div id="frame_colors_<?=$arResult['ID'];?>" class="frame_colors">
	<?foreach($prop_colors  as $prop_colors_code){?>
		<?if(!empty($arResult['PROPERTIES'][$prop_colors_code]['VALUE'])):?>
		<div id="ton_colors_block_<?=$arResult['ID'];?>" class="block_colors_code">
			<div class="block_colors_code_name"><?=$arResult['PROPERTIES'][$prop_colors_code]['NAME']?>:</div>
						<?$arFilter = Array("IBLOCK_ID"=>'31', "ID" =>  $arResult['PROPERTIES'][$prop_colors_code]['VALUE']);
						$res = CIBlockElement::GetList(Array("SORT"=>"ASC"), $arFilter, false, false, $arSelect);
						$iii = 0;
