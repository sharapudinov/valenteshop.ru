...
<?if($arResult['PROPERTIES']['PRICE_FADE']['VALUE']){?>
	<style>body .middle_info{display:none; !important}</style>
<?}?>
<?/*$pge = $APPLICATION->GetCurPage();
if (preg_match("/roca_roka/i", $pge) || preg_match("/roca3/i", $pge) || preg_match("/roca/i", $pge)) {?>
	<style>.middle_info .block_price_add{display:none;}.top_info_text, .top_info_text span{font-size:16px !important;}</style>
<?}*/?>
		...
		<div class="item_info">
			...
			<div class="right_info">
				...
				if( $arStock = $rsStock->GetNext() ){?>
					<a href="/sale/"><img src="/bitrix/images/knopka.png" /></a>
				<?}?>
			...
			<form id="my_form_<?=$arResult['ID']?>" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
				<div class="middle_info">
					...
					<?$SELECTPROP = array();$PROPERTY_SKU = array('CVETARAKOVINA', 'CVETASTOL', 'KORPUS', 'FASAD', 'PLANKA', 'KOMPLECT');$cli = 0;?>
					<?if(empty($arResult["OFFERS"])){?>
					<table id="table_sku_<?=$arResult['ID']?>" class="block_price_add table_sku_default" cellspacing="0" cellpadding="0" border="0" width="100%"><tr><td>
						...
						<nobr>
							...
							<a style="display:none1;" rel="nofollow" element_id="#<?=$arResult["ID"];?>" href="<?=$arResult["ADD_URL"]?>" onclick="return addToCart_new((this, 'detail', '<?=GetMessage("CATALOG_IN_CART")?>', 'cart', '<?=$arParams["BASKET_URL"]?>', '<?=$arResult["ID"]?>');" class="button add_item offers_cart" alt="<?=$arResult["NAME"]?>">
									<span><?=GetMessage('CATALOG_ADD_TO_BASKET')?></span>
								</a>
								<a style="display:none;" rel="nofollow" element_id="#<?=$arResult["ID"];?>" href="<?=$arResult["ADD_URL"]?>" onclick="return addToCart_my2(this, 'detail', '<?=GetMessage("CATALOG_IN_CART")?>', 'cart', '<?=$arParams["BASKET_URL"]?>', '<?=$arResult["ID"]?>');" class="button add_item property_cart" alt="<?=$arResult["NAME"]?>">
									<span><?=GetMessage('CATALOG_ADD_TO_BASKET')?></span>
								</a>
				...
				<?foreach($arResult["OFFERS"] as $PROP){/*print_r($PROP);*/
					if($cli++ == 0){
						$class = "block_price_add table_sku_default table_sku_prop";
					}else{
						$class = "block_price_add table_sku_default table_sku_prop table_sku_prop_none";
					}
					foreach($PROPERTY_SKU as $PROP_SKU){
						if($PROP['DISPLAY_PROPERTIES'][$PROP_SKU]['LINK_ELEMENT_VALUE']){
							foreach($PROP['DISPLAY_PROPERTIES'][$PROP_SKU]['LINK_ELEMENT_VALUE'] as $PROPVAL_ID){
								$class .= " propcuselFrame-sels".$PROP_SKU."_".$arResult['ID'].$PROPVAL_ID['ID'];
							}
						}
					}?>
					<table class="<?=$class;?>" cellspacing="0" cellpadding="0" border="0" width="100%"><tr><td>
						...
						<?if( !empty($PROP["PRICES"] )){?>
							...
							?>
							<div class="price_block<?if(!$PROP["CAN_BUY"]):?> bottom20<?endif;?>">
						...
						<nobr>
							...
							<?endif;?>
							<!--noindex-->
							<?if( $arResult["OFFERS"]["0"]["CAN_BUY"] ){?>

								<a rel="nofollow" element_id="#<?=$PROP["ID"];?>" href="<?=$arResult["OFFERS"]["0"]["ADD_URL"]?>" onclick="return addToCart_new(this, 'detail', '<?=GetMessage("CATALOG_IN_CART")?>', 'cart', '<?=$arParams["BASKET_URL"]?>', '<?=$arResult["ID"]?>', '<?=$PROP["ID"]?>');" class="button add_item">
								<span><?=GetMessage('CATALOG_ADD_TO_BASKET')?></span>
								</a>
								</td>
								<td class="shadow"><i class="shadow_right"></i></td>
								<td class="one-click">
								<a id="one_click_buy_open" class="one_click_buy button" onclick="return oneClickBuy('<?=$PROP["ID"];?>');">
									<span><?=GetMessage('ONE_CLICK_BUY')?></span>
								</a>
							<?}?>
							<!--/noindex-->	
...
/*foreach($proper as $propval){
	if (!preg_match("/_sku/i", $propval) && !preg_match("/_check/i", $propval)) {
...
<?foreach($arResult["PRODUCT_PROPERTIES"] as $pid => $product_property):?>
							...
							<tr valign="top">
								...
								<div class="lineForm">
									<?if(!empty($arResult['PROPERTIES'][$pid]['VALUE'])){?>
										<select class="sel80" id="COLORS_<?=$arResult['ID']?>" name="<?echo $arParams["PRODUCT_PROPS_VARIABLE"]?>[<?echo $pid?>][val]">
								...
								<div class="lineForm">
								<?if(!empty($arResult['PROPERTIES'][$pid]['VALUE'])){?>
									<select id="<?=$pid;?>_<?=$arResult['ID']?>" class="sel80" name="<?echo $arParams["PRODUCT_PROPS_VARIABLE"]?>[<?echo $pid?>][val]">
									<?foreach($arResult['PROPERTIES'][$pid]['VALUE'] as $pr_key => $pr_val){?>
...
<? endif;?>
		...
		foreach($SELECTPROP as $PROPkey=>$PROPval){?>
			<tr valign="top">
				<td>
					<span id="namesels<?=$PROPkey;?>_<?=$arResult['ID']?>"><?=$PROPval['name']?>:</span>
					<input type="hidden" name="prop[<?=$PROPkey?>][name]" value="<?=$PROPval['name']?>" />
				</td>
				<td>
					<div class="lineForm check_select propchek">
						<select id="sels<?=$PROPkey;?>_<?=$arResult['ID']?>" class="sel80" name="prop[<?echo $PROPkey?>][val]">
						<?$idcolor = "";?>
						<?foreach($PROPval['prop'] as $prop_k=>$prop_v){?>
							<?if($arResult['OFFERS']['0']['DISPLAY_PROPERTIES']['CVETASPISOK']['VALUE'][$ip] == $prop_v['ID']){?>
			...
			<tr valign="top">
                ...
                <?if ($arResult["SECTION"]["NAME"] != 'Унитазы'){
                    if ($arResult["SECTION"]["PATH"]["0"]["NAME"] != 'Ванны'){?>
				<div class="colors_boxbox">		
					<div id="s<?=$PROPkey;?>_<?=$arResult['ID']?>" class="button_colors_block button_colors_click cuselFrame-sels<?=$PROPkey;?>_<?=$arResult['ID']?>">		
	...
	<?}?>
			...
			<div class="top_info">
				...
				<?else:?>
					...
					<?endif;?>
				<?endif;?>
							<div class="available_block">Наличие уточняйте у менеджера</div>
...
<?
$IBLOCK_ID_PROP = 15;
$properties = CIBlockProperty::GetList(Array("sort"=>"asc", "name"=>"asc"), Array("ACTIVE"=>"Y", "IBLOCK_ID"=>$IBLOCK_ID_PROP, "CODE"=>"%_CHECK"));
while ($prop_fields = $properties->GetNext())
{
...
<?if(!empty($arResult['PROPERTIES'][$prop_vib_code]['VALUE']) && in_array($prop_vib_code, $proper)):?>
<?$cart_i = 1;?>
	<?if($arResult['PROPERTIES'][$prop_vib_code]['PROPERTY_TYPE'] == "S"):?>
		<h3><?=$arResult['PROPERTIES'][$prop_vib_code]['NAME']?></h3>
		<?if($arResult['PROPERTIES'][$prop_vib_code]['HINT'] == "radio" || empty($arResult['PROPERTIES'][$prop_vib_code]['HINT'])):?>
		<?foreach($arResult['PROPERTIES'][$prop_vib_code]['VALUE'] as $propelid_key => $propelid){?>
			<div><input class="compl_checkradio" type="radio" name="prop[<?=$prop_vib_code?>]" value="<?=$arResult['PROPERTIES'][$prop_vib_code]['DESCRIPTION'][$propelid_key]?>/<?=$propelid?>" id="<?=$prop_vib_code?>_<?=$arResult['PROPERTIES'][$prop_vib_code]['PROPERTY_VALUE_ID'][$propelid_key]?>" /><label for="<?=$prop_vib_code?>_<?=$arResult['PROPERTIES'][$prop_vib_code]['PROPERTY_VALUE_ID'][$propelid_key]?>"><?=$propelid?></label></div>
		<?}?>
		<?elseif($arResult['PROPERTIES'][$prop_vib_code]['HINT'] == "checkbox"): ?>
		<?foreach($arResult['PROPERTIES'][$prop_vib_code]['VALUE'] as $propelid_key => $propelid){?>
			<div><input class="compl_checkradio" type="checkbox" name="prop[<?=$prop_vib_code?>][]" value="<?=$arResult['PROPERTIES'][$prop_vib_code]['DESCRIPTION'][$propelid_key]?>/<?=$propelid?>" id="<?=$prop_vib_code?>_<?=$arResult['PROPERTIES'][$prop_vib_code]['PROPERTY_VALUE_ID'][$propelid_key]?>" /><label for="<?=$prop_vib_code?>_<?=$arResult['PROPERTIES'][$prop_vib_code]['PROPERTY_VALUE_ID'][$propelid_key]?>"><?=$propelid?></label></div>
		<?}?>
		<br />
		<?endif;?>
	<?elseif($arResult['PROPERTIES'][$prop_vib_code]['PROPERTY_TYPE'] == "E"):?>
		<?if($arResult['PROPERTIES'][$prop_vib_code]['HINT'] == "radio" || empty($arResult['PROPERTIES'][$prop_vib_code]['HINT'])):?>
			<?$proi = 0;?>
			<h3><?=$arResult['PROPERTIES'][$prop_vib_code]['NAME']?></h3>
			<?foreach($arResult['PROPERTIES'][$prop_vib_code]['VALUE'] as $propelid){?>
				...
				if($ob = $res->GetNextElement()){ 
					...
					<? if($proi++ == 0){?>
						<div><input class="compl_checkradio" type="radio" name="prop[<?=$prop_vib_code?>]" value="<?=$arProps['PRICE']['VALUE']?>/<?=$ar_fields['NAME']?>" id="<?=$prop_vib_code?>_<?=$ar_fields['ID']?>" /><label for="<?=$prop_vib_code?>_<?=$ar_fields['ID']?>">
						<?if(!empty($ar_fields['PREVIEW_PICTURE'])){
							...
							$hgt = $file['height']/2 - 2;
							echo '<img style="top:'.$hgt.'px" class="prop_picture" border="0" src="'.$file["src"].'" width="'.$file["width"].'" height="'.$file["height"].'" alt="'.$arResult["NAME"].'" title="'.$arResult["NAME"].'" />';
						}?>
			...
			<?foreach($arResult['PROPERTIES'][$prop_vib_code]['VALUE'] as $propelid){?>
				...
				if($ob = $res->GetNextElement()){ 
					$ar_fields = $ob->GetFields();  
					 $arProps = $ob->GetProperties();?>
					<? if($proi++ == 0){?>
						<div><input class="compl_checkradio" type="checkbox" name="prop[<?=$prop_vib_code?>][]" value="<?=$arProps['PRICE']['VALUE']?>/<?=$ar_fields['NAME']?>" id="<?=$prop_vib_code?>_<?=$ar_fields['ID']?>" /><label for="<?=$prop_vib_code?>_<?=$ar_fields['ID']?>"><?=$ar_fields['NAME']?></label></div>
					<?}else{?>
						<div><input class="compl_checkradio" type="checkbox" name="prop[<?=$prop_vib_code?>][]" value="<?=$arProps['PRICE']['VALUE']?>/<?=$ar_fields['NAME']?>" id="<?=$prop_vib_code?>_<?=$ar_fields['ID']?>" /><label for="<?=$prop_vib_code?>_<?=$ar_fields['ID']?>"><?=$ar_fields['NAME']?></label></div>
					<?}?>
				<?}?>
...
<? }?>
<?if($cart_i == 1){?>
	<style>.offers_cart{display:none;}</style><script>$(document).ready(function(){ $('.property_cart').css('display','inline-block'); });</script>
<?}else{?>
	<style>.offers_cart{display:inline-block;}.property_cart{display:none;}</style>
<?}?>
</div>
							...
							<?foreach ($arResult["SKU_ELEMENTS"] as $key => $arSKU){?>
								<tr>
									...
									<!--noindex-->
										<?if( $arSKU["CAN_BUY"] ){?>
											<td class="buy_link">
												<a rel="nofollow" element_id="#<?=$arSKU["ID"];?>" href="<?=$arSKU["ADD_URL"]?>" onclick="return addToCart_new(this, 'detail', '<?=GetMessage("CATALOG_IN_CART")?>', 'cart', '<?=$arParams["BASKET_URL"]?>', '<?=$arResult["ID"]?>', '<?=$arSKU["ID"]?>');"><?=GetMessage("CATALOG_BUY")?></a>
											</td>
											<td class="buy_link">
		...
		<?if ($showProps):?>
			...
			<div class="char-wrapp">			
				<?foreach( $arResult["DISPLAY_PROPERTIES"] as $arProp ){?>
					...
					<?if (($arProp["CODE"]!="HIT")&&($arProp["CODE"]!="RECOMMEND")&&($arProp["CODE"]!="PROP")&&($arProp["CODE"]!="NEW")&&($arProp["CODE"]!="OPTIONS")&&($arProp["CODE"]!="STOCK")):?>				
...
<?$APPLICATION->IncludeComponent(
	...
	Array(
		...
		"OFFERS_PROPERTY_CODE" => $PROPERTY_SKU,
		"OFFERS_LIMIT" => "500",
		"PRICE_CODE" => $arParams['PRICE_CODE'],
		"USE_PRICE_COUNT" => "N",
		"SHOW_PRICE_COUNT" => "1",
		"PRICE_VAT_INCLUDE" => "Y",
		"PRODUCT_PROPERTIES" => array("PERFOMANCE", "COLORS", "PRIMER_SKU"),
