			...
			<tbody>
				...
				*/?>
				...
				<?endif?>
					...
					<tr>
						<td>Изменить платежную систему:</td>
						<td>
<form action="" method="POST">	
<input name="ORDER_ID" type="hidden" value="<?=$arResult['ID']?>" />

<?
// Выведем все активные платежные системы для текущего сайта, для типа плательщика с кодом 2, работающие с валютой RUR
$db_ptype = CSalePaySystem::GetList($arOrder = Array("SORT"=>"ASC", "PSA_NAME"=>"ASC"), Array("ACTIVE"=>"Y", "PERSON_TYPE_ID"=>3));
while ($ptype = $db_ptype->Fetch())
{
   if($ptype["ID"] == $arResult['PAY_SYSTEM_ID']){$bFirst = True;}
   ?><input class="pay_type_my" id="pay_<?=$ptype["ID"]?>" type="radio" name="PAY_SYSTEM_ID" value="<?echo $ptype["ID"] ?>"<?if ($bFirst) echo " checked";?>><label style="cursor:pointer;padding:0px 0 0 5px;" for="pay_<?=$ptype["ID"]?>"><?echo $ptype["PSA_NAME"] ?></label><br><?
   $bFirst = false;
