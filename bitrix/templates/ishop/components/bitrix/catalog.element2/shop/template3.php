...
<?foreach($prop_sku  as $prop_sku_code){?>
	...
	<select id="<?=$prop_sku_code;?>_<?=$arResult['ID']?>" class="sel80" name="<?echo $prop_sku_code?>">
		...
		while($ob = $res->GetNextElement()){
			...
			<?if($iii++ == 0){?>
				<option value="<?=$arProps['PRICE']['VALUE']?>" name="<?=$prop_sku_code?>" selected><?=$ar_fields['NAME']?></option>
			<?}else{?>
				<option value="<?=$arProps['PRICE']['VALUE']?>" name="<?=$prop_sku_code?>"><?=$ar_fields['NAME']?></option>
			<?}?>
		<?}?>
	</select>
	</div>
