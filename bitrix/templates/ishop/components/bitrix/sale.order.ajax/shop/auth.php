...
<table border="0" cellspacing="0" cellpadding="1">
	...
	<tr>
		<td valign="top">
			<form class="ishop auth" method="post" action="" name="order_auth_form">
				<?=bitrix_sessid_post()?>
				<?
				...
				<table class="sale_order_full_table">
					...
					<tr>
						<!--noindex-->
							<td nowrap><a rel="nofollow" href="<?=$arParams["PATH_TO_AUTH"]?>?forgot_password=yes&back_url=<?= urlencode($APPLICATION->GetCurPageParam()); ?>"><?echo GetMessage("STOF_FORGET_PASSWORD")?></a></td>
						<!--/noindex-->
					</tr>
					<tr>
						<td nowrap>
							<button class="button2" type="submit" value="<?echo GetMessage("STOF_NEXT_STEP")?>"><span><?echo GetMessage("STOF_NEXT_STEP")?></span></button>
		...
		<td valign="top">
			<?if($arResult["AUTH"]["new_user_registration"]=="Y"):?>
				<form class="ishop auth" method="post" action="" name="order_reg_form">
					<?=bitrix_sessid_post()?>
					<?
					foreach ($arResult["POST"] as $key => $value)
					{
