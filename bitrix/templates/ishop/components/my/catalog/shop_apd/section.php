...
<?if($USER->IsAdmin()) var_dump($template);
...
if( $count_sections > 0 ){?>
	<div class="container left shop">
		<div class="inner_left">
			<div id="main_display_table" class="display_table">
...
<?}else{?>
	...
	false
		...
		<?$APPLICATION->IncludeComponent(
			...
			"shop",
			Array(
				"VIEWED_CURRENCY"=>$arParams["CURRENCY_ID"],
				"VIEWED_COUNT" => "2",
				"VIEWED_NAME" => "Y",
				"VIEWED_IMAGE" => "Y",
				"VIEWED_PRICE" => "N",
	...
	<div class="container left">
		<div class="inner_left">
			<div class="sort_header">
				<!--noindex-->
					<?$display = array_key_exists("display", $_REQUEST) && !empty($_REQUEST["display"]) ? $_REQUEST["display"] : 'table' ;?>
					<div class="sort_filter">
						...
						foreach ($arAvailableSort as $key => $val){
							...
							<a rel="nofollow" href="<?=ToLower($APPLICATION->GetCurPageParam('sort='.$key.'&order='.$newSort, 	array('sort', 'order', 'mode')))?>" class="button_middle <?=$sort == $key ? 'current' : ''?> <?=$sort_order?> <?=$key?>" rel="nofollow">
			...
			<?$template = "shop_";
			if( !empty($_REQUEST["display"]) && $_REQUEST["display"] == "list" ){
				$template .= "list";
			}else{
				$template .= "table_apd";
			}?>
