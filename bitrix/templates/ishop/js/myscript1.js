$(document).ready(function(){
	...
	$('.pay_type_my').live('click', function(){
		var form = $(this).parents('form').serialize();
       	$.ajax({
       		type: 'POST',
			url: "/ajax/pay_type.php",
          	data: form,
       		success: function(data) {
				$('.inside_page_content_ajax').html(data);
          	},
...
*/
	/*$('.check_select span').live('click', function(){	
		var form = $(this).parents('form').serialize();
       		$.ajax({
         		 type: 'POST',
				url: "/ajax/sku.php",
	...
	$('.check_select span').live('click', function(){	
		...
		var select = $(this).parents('.cuselScrollArrows').attr('id');
		$('.button_colors_click.' + select).html('<div class="button_colors">Выберите цвет</div>');
		var clas = "";
		$('#' + parent + ' .propchek span.cuselActive').each(function (){
			var pri_val = $(this).attr('val');
			var pri_id = $(this).parents('.cuselScrollArrows').attr('id');
			clas = clas + '.prop' + pri_id + pri_val;
		});
		$('#' + parent + ' .table_sku_prop').css('display','none');
		$('#' + parent + ' ' + clas).css('display','table');
		$('#' + parent + ' .list_item ' + clas).css('display','inline-block');
	});
	...
	$('.button_colors_click').live('click', function(){
		...
		var idname = $('#namesel' + id).html();
       	$.ajax({
         	type: 'POST',
			url: "/ajax/colors.php",
          	data: ({id : idprop, ids : id, idname: idname}),
          	success: function(data) {
	...
	$('#nabor_complect .compl_checkradio').live('click', function(){	
		var id = $(this).parents('form').attr('id');	
